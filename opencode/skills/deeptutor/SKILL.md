...
A dedicated, already-initialized DeepTutor home exists at `/home/akash/Work/my-deeptutor`
(configured: deepseek LLM, openrouter embeddings, duckduckgo search). Run `deeptutor`
commands from that directory (or pass `--home /home/akash/Work/my-deeptutor` where
supported) so knowledge bases, sessions, and memory persist in that workspace.
DeepTutor is installed globally on PATH (v1.5.16).

When output will be parsed by you programmatically, always pass `--format json` and
read line-by-line JSON events; capture `session_id` from the `done` event for
continuity.
...
## Session Handoff (chaining capabilities)

The most powerful agent pattern: chain capability calls within **one session** so each
new turn inherits chat history, attached KBs, tools, language preference, and memory.

```bash
cd /home/akash/Work/my-deeptutor

# Step 1 — start a session, capture its id from the 'done' event
SESSION_ID=$(deeptutor run deep_research "Survey recent papers on RAG" \
  --tool web_search --kb papers \
  --config mode=report --config depth=standard \
  --format json | jq -r 'select(.type=="done") | .session_id')

# Step 2 — summarize in the same session with a different capability
deeptutor run chat "Summarize the top three findings as bullet points" \
  --session "$SESSION_ID" --format json

# Step 3 — quiz on the same session's content
deeptutor run deep_question "Quiz me on the findings from this session" \
  --session "$SESSION_ID" --config num_questions=5 --format json
```

If `$SESSION_ID` comes back empty, re-read the raw JSON stream (`--format json`) to
inspect what happened before retrying.

## JSON Event Parsing

With `--format json`, output is one JSON event per line. Event types include
`content`, `tool_call`, `stage_end`, `error`, and `done`.

```bash
# Join all content chunks into the final answer
deeptutor run chat "Hello" --format json | \
  jq -s 'map(select(.type=="content") | .content) | join("")'

# Extract the session id for later --session reuse
deeptutor run chat "Hello" --format json | \
...
Python equivalent (line-by-line):

```python
import json, subprocess

proc = subprocess.Popen(
    ["deeptutor", "run", "deep_solve", "Find d/dx [sin(x^2)]",
